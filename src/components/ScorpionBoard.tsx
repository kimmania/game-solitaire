...
export function ScorpionBoard({
  ...
  const handleStock = () => {
    onClearSelection();
    onDispatch({ kind: 'deal-scorpion-stock' });
  };
  ...
  const canDeal = state.stock.length > 0 && !hasEmptyTableau(state);

  return (
    <div className="board board--scorpion">
      <div className="board__top board__top--scorpion">
        <button
          type="button"
          ...
          aria-label={
            state.stock.length === 0
              ...
              : hasEmptyTableau(state)
                ...
                : `Deal to first four columns (${state.stock.length} left)`
          }
        >
          {state.stock.length > 0 ? (
            <>
        ...
        <div className="spider-progress" aria-live="polite">
          ...
          <span className="spider-progress__count">
            {state.foundations} / {SCORPION_FOUNDATIONS}
          </span>
        </div>
      </div>
