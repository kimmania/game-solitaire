...
import {
  buildEightOffDropAction,
  getEightOffAutoFoundation,
  tableauSelectableRange,
} from '../game/eightoff/rules';
import { EIGHT_OFF_SLOTS } from '../game/eightoff/types';
...
export function EightOffBoard({
  ...
  const handleFreeCell = (slotIndex: number) => {
    ...
    if (card && selection && pileKey(selection.from) === pileKey(ref)) {
      if (getEightOffAutoFoundation(state, ref, 0)) {
    ...
    if (selection) {
      const drop = buildEightOffDropAction(state, selection, slotIndex);
      if (drop) {
        onDispatch(drop);
        return;
      }
  ...
  const handleTableauCard = (colIndex: number, cardIndex: number) => {
    ...
    if (
      ...
      if (getEightOffAutoFoundation(state, from, range.fromIndex + range.count - 1)) {
  ...
  return (
    <div className="board board--eightoff">
      <div className="board__top board__top--eightoff">
        <div className="board__freecells board__freecells--8" aria-label="Cells">
          {Array.from({ length: EIGHT_OFF_SLOTS }, (_, i) => {
